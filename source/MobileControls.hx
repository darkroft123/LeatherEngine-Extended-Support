...
class MobileControls extends FlxTypedSpriteGroup<FlxSprite>
{
    ...
    public function generateButtons(kc:Int = 4, dodges:Bool = false)
    {
        ...
        var colors:Array<FlxColor> = [0xff1a9977, 0xffdacd21, 0xff2cee00, 0xffef2917];
        for (i in 0...keyCount)
        {
            var color:FlxColor = colors[i%4];
             var box:FlxSprite = new FlxSprite(i*w, 0).makeGraphic(w, 720, color);
            box.alpha = Options.getData("mobileCAlpha");
            add(box);
            buttons.push(box);
        }
    ...
    public function updateInput()
    {
        ...
        for (i in 0...keyCount)
        {
            ...
            var mobileAlpha:Float = Options.getData("mobileCAlpha");

            if (released[i])
            {
                if (buttons[i] != null)
                    buttons[i].alpha = mobileAlpha; // controlado por el menú
            }

            if (pressed[i] || justPressed[i])
            {
                if (buttons[i] != null)
                    buttons[i].alpha = 0.2; // valor fijo para presionado
            }

        }
    }

    private function createHitboxGraphic(Width:Int, Height:Int, Color:Int = 0xFFFFFF):BitmapData {
        var shape:Shape = new Shape();
        var type:String = Options.getData("hitboxType");
        if (type == "Hidden") {
            var btmpHidden = new BitmapData(Width, Height, true, 0x00000000);
            return btmpHidden;
        }
        if (type == "No Gradient") {
    ...
    else if (type == "No Gradient (Old)") {
        shape.graphics.lineStyle(10, Color, 0.8);
        shape.graphics.drawRect(0, 0, Width, Height);
        shape.graphics.endFill();
    }
    ...
    public static function verticalPressOptions(grp:FlxTypedGroup<Option>, curSelected:Int, ?camera:FlxCamera)
    {
        ...
        for (touch in FlxG.touches.list)
        {
            if (touch.justPressed)
            {
                var selectedAlphabet = grp.members[curSelected].alphabetText;
