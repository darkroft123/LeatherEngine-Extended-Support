...
class FlxVideo extends FlxBasic {
	...
	public function onVideoReady()
	{
		...
		var videoW = vlcBitmap.videoWidth;
		var videoH = vlcBitmap.videoHeight;

		var scale = Math.min(sw / videoW, sh / videoH);

		vlcBitmap.set_width(videoW * scale);
		vlcBitmap.set_height(videoH * scale);

		// Centrar el video
		vlcBitmap.x = (sw - vlcBitmap.width) / 2;
		vlcBitmap.y = (sh - vlcBitmap.height) / 2;
